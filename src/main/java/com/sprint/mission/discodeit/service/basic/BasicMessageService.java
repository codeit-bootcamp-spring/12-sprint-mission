...
public class BasicMessageService implements MessageService {
  ...
  public MessageDto create(MessageCreateRequest messageCreateRequest,
    ...
    Channel channel = channelRepository.findById(channelId)
        .orElseThrow(
            () -> {
              log.warn("메시지 생성 실패 - {}: channelId={}", ErrorCode.CHANNEL_NOT_FOUND.getMessage(),
                  channelId);
    ...
    User author = userRepository.findById(authorId)
        .orElseThrow(
            () -> {
              log.warn("메시지 생성 실패 - {}: authorId={}", ErrorCode.USER_NOT_FOUND.getMessage(),
                  authorId);
  ...
  public MessageDto update(UUID messageId, MessageUpdateRequest request) {
    ...
    Message message = messageRepository.findById(messageId)
        .orElseThrow(
            () -> {
              log.warn("메시지 수정 실패 - {}: id={}", ErrorCode.MESSAGE_NOT_FOUND.getMessage(),
                  messageId);
  ...
  public void delete(UUID messageId) {
    ...
    if (!messageRepository.existsById(messageId)) {
      log.warn("메시지 삭제 실패 - {}: id={}", ErrorCode.MESSAGE_NOT_FOUND.getMessage(), messageId);
