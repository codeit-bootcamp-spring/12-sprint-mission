...
public class JavaApplication {
    public static void main(String[] args) {
        ...
        messageService.save(new Message(u1.getId(), c2.getId(), "그럼 전 프론트엔드 할게요!"));
        ...
        System.out.println("유저2 수정된 닉네임: " + userService.findById(u2.getId()).getNickname());

        // 마지막 메시지 내용 수정 및 확인
        List<Message> allMsgs = messageService.findAll();
        Message lastMsg = allMsgs.get(allMsgs.size() - 1);
        messageService.update(lastMsg.getId(), "전 풀스택 할래요!", null);
        System.out.println("수정된 마지막 메시지: " + messageService.findById(lastMsg.getId()).getContent());

        System.out.println("\n========== [4. 특정 데이터 삭제 검증] ==========");
        // 유저 3의 첫 번째 메시지 삭제
        System.out.println("삭제 전 메시지 수: " + messageService.findAll().size());
        Message targetMsg = allMsgs.get(2); // "저도 같이 공부해요."
        messageService.delete(targetMsg.getId());
        System.out.println("삭제 후 메시지 수: " + messageService.findAll().size() + " (기대값: 9)");
        System.out.println("삭제 확인 (null): " + messageService.findById(targetMsg.getId()));

        System.out.println("\n========== [5. 채널 삭제] ==========");
        // '자바-기초' 채널 삭제 전 상황: 메시지 9개 중 5개(하나 지움)가 이 채널 것
        System.out.println("'자바-기초' 삭제 전 전체 메시지: " + messageService.findAll().size());

        channelService.delete(c1.getId());

        System.out.println("'자바-기초' 삭제 후 남은 메시지: " + messageService.findAll().size() + " (기대값: 4)");
        System.out.println("남아있는 메시지는 모두 '프로젝트-방' 것인가? " +
                messageService.findAll().stream().allMatch(m -> m.getChannelId().equals(c2.getId())));
