...
public class JavaApplication {
	...
	public static User setupUser(UserService userService) {
		...
		User user1 = userService.create(
			new User("user1", "test@email.com", "1234", "user", "+821012345678", "https://icon.com/1.png"));
		System.out.println("사용자 생성: " + user1.getId());
		System.out.println("---------------사용자 테스트 끝!!!---------------------\n");
		return user1;
	}
	...
	static Channel setupChannel(ChannelService channelService, User user) {
		System.out.println("----------------채널 테스트 시작!!!------------------");
		Channel channel = channelService.create(new Channel(ChannelType.TEXT, "공지", "공지 채널입니다.", user.getId()));
		System.out.printf("채널 생성: %s, 이름: %s\n", channel.getId(), channel.getName());
		System.out.println("----------------채널 테스트 끝!!!------------------");
		return channel;
	}

	static void messageCreateTest(MessageService messageService, Channel channel, User author) {
		Message message = messageService.create(channel, new Message(author.getId(), channel.getId(), "안녕하세요."));
